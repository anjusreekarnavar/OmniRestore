...
def dataprocessing(input,target,args):
        image_size=41
        new_inputlr=[]
        new_inputhr=[]
        ...
        input=input.cpu()
        target=target.cpu()
        for (data1,data2) in zip(input,target):
            lr_images,hr_images=convert_cv(data1,data2)
            lr_y_image = bgr2ycbcr(lr_images, use_y_channel=True)
            hr_y_image = bgr2ycbcr(hr_images, use_y_channel=True)
            lr_image,hr_image=random_crop(lr_y_image,hr_y_image,image_size)
            ...
            lr_image,hr_image=random_vertically_flip(lr_image,hr_image,p=0.5)
        
            input=image2tensor(lr_image, range_norm=False, half=False)
            target=image2tensor(hr_image,range_norm=False, half=False)
            
            new_inputlr.append(input)
            new_inputhr.append(target)
        input=torch.stack(new_inputlr)
        target=torch.stack(new_inputhr)
        return input,target
def valprocessing(input,target,args):
        image_size=41
        new_inputlr=[]
        new_inputhr=[]
        device = torch.device(args.device)
        ...
        for (data1,data2) in zip(input,target):
            ...
            lr_y_image = bgr2ycbcr(lr_images, use_y_channel=True)
            hr_y_image = bgr2ycbcr(hr_images, use_y_channel=True)
            lr_image,hr_image=center_crop(lr_y_image,hr_y_image,image_size)
        ...
        target=torch.stack(new_inputhr)
        return input,target
def testprocessing(input,target,args):
        new_inputlr=[]
        new_inputhr=[]
        device = torch.device(args.device)
        upscale_factor=2
        ...
        for (data1,data2) in zip(input,target):
            ...
            new_inputlr.append(lr_y_tensor)
            new_inputhr.append(hr_y_tensor)
        input=torch.stack(new_inputlr)
        target=torch.stack(new_inputhr)
        return input,target
def convert_cv(input,target):
...
def center_crop(lr_image: np.ndarray, hr_image: np.ndarray, image_size: int) -> [np.ndarray, np.ndarray]: # type: ignore
...
def random_horizontally_flip(lr_image: np.ndarray, hr_image: np.ndarray, p=0.5) -> [np.ndarray, np.ndarray]: # type: ignore
