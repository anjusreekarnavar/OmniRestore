...
class Decoder(nn.Module):
    ...
    def forward_loss(self, target, pred):
        ...
        loss = ((pred - target) ** 2 + epsilon)
       
        loss = loss.mean()          
       
        return loss

    def forward(self, clean_imgs,latent,ids_restore,mask,args):
  
        out=[]
        # normalization
        
        self.mean = self.mean.to(clean_imgs.device)
        self.std = self.std.to(clean_imgs.device)
        clean_imgs = (clean_imgs - self.mean) / self.std
    
        pred = self.forward_decoder(latent,ids_restore)  # [N, L, p*p*3]
        out.append(pred)
        loss = self.forward_loss(clean_imgs, pred)
        out.append(loss)
        return  out
